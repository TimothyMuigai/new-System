...
function SubGraph({ newData }) {
    const { getSubCategory } = useContext(SubscriptionContext);
    ...
    const filteredChartData = filterByTimeRange(
        selectedCategory === 'All'
          ? newData
          : newData.filter(item => item.sub_Category === selectedCategory),
        selectedTimeRange
      );    
    ...
    const formatIncomeForChart = (data) => {
        ...
        if (selectedTimeRange === 'This Year') {
          ...
          data.forEach(item => {
            ...
            monthlyTotals[monthLabel] += parseFloat(item.amount);
          });
      
          return Object.entries(monthlyTotals).map(([date, subs]) => ({ date, subs }));
        }
    ...
    const totalIncome = filteredChartData.reduce((sum, item) => sum + parseFloat(item.amount), 0);
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-100">Subscription Overview</h2>
